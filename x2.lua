...
local IDLE_WAIT_NO_KEY = 3
local IDLE_WAIT_RETRY = 1.5
...
local function getDomainBirdcageAmount()
...
local function tpToPortalAndEnter(prompt)
    ...
    if not attachment or not attachment:IsA("Attachment") then
        warn("portal attachment not found")
        return false
    end
...
local function openPortalByNpcAndEnter()
    local bird = getDomainBirdcageAmount()
    if bird <= 0 then
    ...
    if not prompt then
        warn("portal did not appear after talking NPC")
        return false, nil
    end
    ...
    warn("failed to enter boss map after opening portal")
    return false, nil
end

local function ensurePortalThenEnter()
    local okExisting, existingBoss = tryEnterExistingPortal()
    if okExisting then
        return true, existingBoss
    end

    return openPortalByNpcAndEnter()
end
...
local function teleportToBossAndHold(bossRoot)
    ...
    if not root or not hum or not bossRoot then return false end
    ...
    while true do
        ...
        if activeBoss then
            ...
            if portalPrompt then
                ...
                local entered, bossFolder = ensurePortalThenEnter()
                if entered and bossFolder then
                    attackBoss(bossFolder)
                    task.wait(AFTER_BOSS_DEAD_DELAY)
                else
                    task.wait(IDLE_WAIT_RETRY)
                end
            else
                local bird = getDomainBirdcageAmount()
                ...
                if bird > 0 then
                    ...
                    log("no Domain Birdcage and no portal, waiting...")
                    task.wait(IDLE_WAIT_NO_KEY)
                end
            end
        end
    end
